...
document.addEventListener("DOMContentLoaded", function () {
    let withdrawButtons = document.querySelectorAll(".withdraw-btn");

    withdrawButtons.forEach(button => {
        button.addEventListener("click", function (event) {
            event.preventDefault(); // Prevent immediate navigation

            let confirmAction = confirm("Are you sure you want to withdraw this complaint?");
            if (confirmAction) {
                setTimeout(() => {
                    window.location.href = button.href;
                }, 50); // delay before redirection
            }
           // let msg = confirm("Complaint withdrawn successfully!");
        });
    });
});
